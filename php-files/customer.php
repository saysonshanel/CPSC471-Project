...
<img src="logo.png" alt="Toyota Logo" width="500" height="150">
   ...
   <body>
      ...
      <h2>Welcome, John Smith! | <a href ="logout.php">Sign Out</a></h2>
   </body>
...
<?php
    //session_start();
    $cust = 888444000;  //hardcoded customer sin
  ...
  $query2 = "SELECT LEASE, FINANCE FROM contracts WHERE CSIN = " . $cust;
   $sqlquery2 = $db->query($query2);
   
   if($sqlquery2->num_rows>0){
       while($row=$sqlquery2->fetch_assoc()){
           echo "You are on a " . $row["LEASE"] ." year lease<br>";
           echo "Your finance amount dropped to $".number_format($row["FINANCE"])."<br><br>";
       }
   }
       else {
...
<form action="customer.php" method="post">
  ...
  <input type="submit" name ="submit_button">
    ...
    if ($db->query($sql) === TRUE) {
        ...
        echo "Updated phone number to " . $phone;
    }
    else {
